...
class Upload 
{
    ...
    private function setDriver($driver = null, $config = null){
        $driver = $driver ? : ($this->driver       ? : \think\Config::get('upload.type'));
        $config = $config ? : ($this->driverConfig ? : \think\Config::get('upload.driverConfig'));
        $class = strpos($driver,'\\')? $driver : '\\org\\upload\\'.ucfirst(strtolower($driver));
    ...
    private function check($file) {
        ...
        if (!$this->checkExt($file['ext'])) {
            $this->error = '請上傳正確的圖檔格式，暫時只支持jpg,gif,png';
            return false;
        }
    ...
    private function getSaveName($file) {
        ...
        if (empty($rule)) { //保持文件名不变
            ...
            $savename = $filename;
        } elseif (in_array($rule, ['md5_file', 'sha1_file'])) {
            $savename = $rule($file['tmp_name']);
        } else {
